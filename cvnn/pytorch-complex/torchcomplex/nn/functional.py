...
def group_norm(input, num_groups, weight=None, bias=None, eps=1e-5, naive=True):
    complex_weight = not(type(weight) == torch.nn.ParameterList)
    if naive:

        real = F.group_norm(input.real, num_groups,
                             weight.real if complex_weight else weight[0], bias.real if complex_weight else bias[0], eps)
        imag = F.group_norm(input.imag, num_groups,
                             weight.imag if complex_weight else weight[1], bias.imag if complex_weight else bias[1], eps)
