...
class _ConvNd(Module):
    ...
    def __init__(self,
        ...
        if complex_weights:
            if transposed:
                self.weight = Parameter(torch.Tensor(
                    in_channels, out_channels // groups, *kernel_size).to(torch.cfloat))
            else:
                self.weight = Parameter(torch.Tensor(
                    out_channels, in_channels // groups, *kernel_size).to(torch.cfloat))
        else:
            if transposed:
                weight_real = Parameter(torch.Tensor(
        ...
        if bias:
            if complex_weights:
                self.bias = Parameter(torch.Tensor(out_channels).to(torch.cfloat))
            else:
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def __init__(
        ...
        super(Conv1d, self).__init__(
            ...
            False, _single(0), groups, bias, padding_mode, complex_weights)

    def _conv_forward(self, input, weight):
        if self.padding_mode != 'zeros':
            return cF.conv1d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            weight, self.bias, self.stride,
                            _single(0), self.dilation, self.groups)
        return cF.conv1d(input, weight, self.bias, self.stride,
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def __init__(
        ...
        super(Conv2d, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias, padding_mode, complex_weights)

    def _conv_forward(self, input, weight):
        if self.padding_mode != 'zeros':
            return cF.conv2d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            weight, self.bias, self.stride,
                            _pair(0), self.dilation, self.groups)
        return cF.conv2d(input, weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also known as the à trous algorithm.
    ...
    def __init__(
        ...
        super(Conv3d, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            False, _triple(0), groups, bias, padding_mode, complex_weights)

    def forward(self, input: Tensor) -> Tensor:
        if self.padding_mode != 'zeros':
            return cF.conv3d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            self.weight, self.bias, self.stride, _triple(0),
                            self.dilation, self.groups)
        return cF.conv3d(input, self.weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also known as the à trous algorithm.
    ...
    def __init__(
        ...
        super(ConvTranspose1d, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            True, output_padding, groups, bias, padding_mode, complex_weights)
