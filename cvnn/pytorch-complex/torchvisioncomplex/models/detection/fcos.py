...
import torchcomplex
import torchcomplex.nn as cxnn
import torchcomplex.nn.functional as cF

class FCOSHead(cxnn.Module):
    """
...
class FCOSClassificationHead(cxnn.Module):
    """
    ...
    def __init__(
        ...
        norm_layer: Optional[Callable[..., cxnn.Module]] = None,
    ) -> None:
        super().__init__()
        ...
        if norm_layer is None:
            norm_layer = partial(cxnn.GroupNorm, 32) #TODO : see if this works

        conv = []
        for _ in range(num_convs // 2):
            conv.append(cxnn.Conv2d(in_channels, in_channels // 2, kernel_size=3, stride=1, padding=1))
            conv.append(norm_layer(in_channels // 2 ))
            conv.append(cxnn.CReLU())
            conv.append(cxnn.Conv2d(in_channels // 2, in_channels, kernel_size=3, stride=1, padding=1))
        ...
        for layer in self.conv.children():
            if isinstance(layer, cxnn.Conv2d):
                cxnn.init.normal_(layer.weight, std=0.01)
                cxnn.init.constant_(layer.bias, 0)

        # NOTE: probably these should be real
    ...
    def forward(self, x: List[Tensor]) -> Tensor:
        ...
        for features in x:
            ...
            cls_logits = (cls_logits.real + cls_logits.imag) / 2 #torch.abs(cls_logits) 
...
class FCOSRegressionHead(cxnn.Module):
    """
    ...
    def __init__(
        ...
        if norm_layer is None:
            norm_layer = partial(cxnn.GroupNorm, 32)
        ...
        # NOTE: these should probably be real
    ...
    def forward(self, x: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        for features in x:
            ...
            bbox_feature = (bbox_feature.real + bbox_feature.imag) / 2 #torch.abs(bbox_feature)
...
class FCOS(cxnn.Module):
    ...
    def __init__(
        ...
        head: Optional[cxnn.Module] = None,
    ...
    def compute_loss(
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            pairwise_match = torch.abs(anchor_centers[:, None, :] - gt_centers[None, :, :]).max(
                dim=2
    ...
    def forward(
        ...
        anchors = [torch.real(anchor) for anchor in anchors]
        # recover level sizes
...
def fcos_resnet18_fpn(
    ...
    **kwargs: Any,
    ...
    if is_trained:
        warnings.warn(
            "Complex-val pretrained models are currently unavailable."
    ...
    norm_layer =  cxnn.BatchNorm2d

    backbone = resnet18(weights=weights_backbone, progress=progress, in_channels=in_channels, norm_layer=norm_layer)
...
def fcos_resnet50_fpn(
    ...
    **kwargs: Any,
    ...
    backbone = resnet50(weights=weights_backbone, progress=progress, in_channels=in_channels, norm_layer=norm_layer)
    backbone = _resnet_fpn_extractor(
        backbone, trainable_backbone_layers, returned_layers=[2, 3, 4], extra_blocks=LastLevelP6P7(256, 256)
    )
    model = FCOS(backbone, num_classes, **kwargs)
