...
def load_SARFishProduct(image_filepath: Path) -> Tuple[
...
def load_SentinelProduct(image_filepath: Path) -> Tuple[
        np.ndarray, np.ndarray, Union[None, int, float], int
    ]:
    """
    ...
    if num_bands != 4:
        raise ValueError('expected 4 bands (i_vh, q_vh, i_vv, q_vv) but got ', num_bands, ' instead.' )

    data_list = []
    

    for i in range(num_bands):
        data_band = Dataset.GetRasterBand(i + 1)
        number_of_data_band_rows, number_of_data_band_columns = get_band_shape(data_band)
    ...
    data_vh = np.flip(data_list[0] + 1j * data_list[1], axis=0)
    data_vv = np.flip(data_list[2] + 1j * data_list[3], axis=0)
    
    
    return data_vh, data_vv